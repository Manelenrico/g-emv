# === v27-PAREJA (PROMPT_72): TODAS las filas sociales APAGADAS ===
# Imagen de ablacion de campo. El cuerpo es el mismo; lo social, ninguno.
...
PARTE_ON = False              # resellado en el 54 (False = candado del 53)
...
VINCULO_M = 0.0              # [impl, calibrado en banco 54]
...
HERIDO_M = 0.0               # [impl, calibrado en banco 54: con 1,5 el don
...
HERIDO_V30 = False            # interruptor de pareado v29<->v30 (banco 56)
...
PROVISION_ON = False          # interruptor de pareado v30<->v31 (banco 59)
...
MANADA_ON = False             # interruptor de pareado v32<->v34 (banco 63)
...
IDENTIDAD_ON = False          # interruptor de pareado v35<->v36 (banco 68)

# ── MIEDO CON MEMORIA (PROMPT_70, sofa del 70 [Manel]) — v37 ────────────────
# El 69 midio que los agresores VUELVEN (138 reencuentros en 80 episodios,
# 186 % de las peleas, separacion mediana 110 tics) y que en el caso exacto
# —vuelve, armado, se acerca— el pega primero solo 7 de 15: por eso NO hay
# golpe preventivo (4 de cada 10 caerian sobre quien no atacaba; una muerte en
# 138 no lo paga; R9). Lo que SI hay es MIEDO: la anima se asusta ANTES del
# golpe al ver venir armado a un agresor CONOCIDO. Ni una linea de conducta
# (R2): lo que salga del miedo —distancia, pared, hermana, nada— lo decide el
# cuerpo. Caducidad de la memoria: TODO EL EPISODIO (el 54 % de los gaps pasa
# de 96 tics; una memoria corta se perderia la mitad de los reencuentros).
REENCUENTRO_CERCA = 5.0      # [impl] a mas de esto, el miedo con memoria calla
REENCUENTRO_MITAD = 0.5      # [impl] fuera del alcance de su arma, la mitad
REENCUENTRO_ON = False        # interruptor de pareado v36<->v37 (banco 70)
# RESELLO (PROMPT_71, sofa del 71 [Manel]): la memoria recuerda el DANO
# ACUMULADO de ese asiento en el episodio, no su ultimo golpe. Era la unica
# palanca honesta que quedaba tras el 70 (la fila se encendia cuando debia y
# no movia ni una decision: 0,18 en una mesa donde ya pesan cuesta, muro y
# anillo). Cuesta cero. Si con ella sigue muda: PARAR y cerrarla con dos
# numeros.
ACUMULADO_ON = False          # interruptor de pareado v37<->v37b (banco 71)
...
ALCANCE_ON = False            # interruptor de pareado v34<->v35 (banco 65)
FUEGO_AMIGO_ON = False        # interruptor de pareado v34<->v35 (banco 65): la
...
REPARTO = {
    ...
    "F-REENCUENTRO":   (0.8, 0.1, 0.1),   # [impl] miedo con memoria (70)
...
class Memoria:
    ...
    # PROMPT_70: la memoria del EPISODIO (sin caducidad): asiento -> {"ultimo",
    # "dano"} de quien nos dano a MI o a la HERMANA (por el asiento del parte
    # v2). Se limpia al empezar episodio, como el resto de la Memoria.
    mem_agresores: dict = field(default_factory=dict)
    dist_agresor: dict = field(default_factory=dict)   # slot -> ultimas dists
    ...
    def observa(self, obs: dict, mundo, tick: int):
        ...
        # PROMPT_70: distancias por asiento (para saber si SE ACERCA). Solo de
        # los que estan en la memoria del episodio; ventana corta de 3 tics.
        if self.mem_agresores:
            _p0 = tuple(you.get("pos") or ())
            _vis0 = {a.get("slot"): a for a in ((obs.get("visible") or {}).get("agents") or [])}
            for _sl3 in list(self.mem_agresores):
                _a3 = _vis0.get(_sl3)
                _q3 = tuple((_a3 or {}).get("pos") or ())
                if _p0 and _q3:
                    _h = self.dist_agresor.setdefault(_sl3, [])
                    _h.append((tick, math.dist(_p0, _q3)))
                    del _h[:-3]
                else:
                    self.dist_agresor.pop(_sl3, None)
                ...
                if PARTE_ON:
                    ...
                    if (p is not None and p["slot"] == mundo.teammate_slot
                            and (self.parte is None
                                 or p["t"] >= self.parte["t"])):
                        # PROMPT_70: el asiento que ella nombra entra en MI
                        # memoria del episodio. El dano conocido: la caida de
                        # hp entre su parte anterior y este (cierto por su
                        # testimonio); si no se puede saber, no se inventa.
                        _sl2 = p.get("agresor_slot")
                        if (REENCUENTRO_ON and p.get("agresor")
                                and _sl2 is not None
                                and _sl2 not in (mundo.slot,
                                                 mundo.teammate_slot)):
                            _caida = 0.0
                            if (self.parte is not None
                                    and self.parte.get("hp") is not None):
                                _caida = max(0.0, float(self.parte["hp"])
                                             - float(p["hp"]))
                            _me = self.mem_agresores.setdefault(
                                _sl2, {"dano": 0.0, "acum": 0.0,
                                       "ultimo": p["t"]})
                            _me["dano"] = max(_me["dano"], _caida)
                            # PROMPT_71: el acumulado de lo que le hizo a ELLA
                            # (cierto: caida de hp entre sus dos partes)
                            _me["acum"] = _me.get("acum", 0.0) + _caida
                            _me["ultimo"] = max(_me["ultimo"], p["t"])
                        self.parte = p
        ...
        for d in (you.get("damage_taken") or []):
            ...
            e["dano"] += cant
            e["ultimo"] = tick
            # PROMPT_70: la memoria del EPISODIO (no caduca). Mi propio
            # `damage_taken.source` es certeza total sobre quien me pego.
            me = self.mem_agresores.setdefault(
                sl, {"dano": 0.0, "acum": 0.0, "ultimo": tick})
            me["dano"] = max(me["dano"], cant)      # su golpe mas fuerte (v37)
            me["acum"] = me.get("acum", 0.0) + cant  # PROMPT_71: el acumulado
            me["ultimo"] = tick
...
def filas(obs: dict, mundo, mem: Memoria, tick: int) -> dict:
    ...
    # ── F-REENCUENTRO (PROMPT_70): el miedo con memoria ──────────────────────
    # Se enciende cuando un asiento de la MEMORIA DEL EPISODIO esta VISIBLE,
    # con ARMA visible que dana, y SE ACERCA (distancia decreciente en >=2
    # tics, o ya a <=2). Magnitud: el miedo que causo su ultimo dano conocido
    # —la MISMA cuenta que F-DANO para ese dano, con la cuesta del 43— escalado
    # por cercania: dentro del alcance de su arma, entera; a <=5, la mitad; mas
    # lejos, nada. SIN LATCH: se recalcula cada tic y se apaga solo. Y SIN
    # DOBLE CUENTA: si ese asiento ya me esta danando (ventana S-7 viva), manda
    # F-DANO y esta calla — el miedo con memoria es para ANTES del golpe.
    F["F-REENCUENTRO"] = 0.0
    if REENCUENTRO_ON and mem.mem_agresores:
        _peor_r, _det_r = 0.0, None
        for _a in (vis.get("agents") or []):
            _sl = _a.get("slot")
            _me = mem.mem_agresores.get(_sl)
            if not _me or _sl in (mundo.slot, mundo.teammate_slot):
                continue
            _act = mem.agresores.get(_sl)
            if _act and (tick - _act["ultimo"]) <= AGRESOR_VENTANA_S * mundo.tick_rate:
                continue                      # ya pega: manda F-DANO
            _mano = _a.get("hand")
            _arma = mundo.items.get(_mano if isinstance(_mano, str)
                                    else (_mano or {}).get("id"))
            if _arma is None or _arma.damage <= 0 or _arma.range < 1:
                continue                      # sin arma visible que dane
            _q = tuple(_a.get("pos") or ())
            if not _q:
                continue
            _d = math.dist(pos, _q)
            if _d > REENCUENTRO_CERCA:
                continue
            _hist = mem.dist_agresor.get(_sl) or []
            _acerca = (_d <= 2.0) or (len(_hist) >= 2
                                      and _hist[-1][1] < _hist[-2][1])
            if not _acerca:
                continue
            _dan_r = float(_me.get("acum", 0.0) if ACUMULADO_ON
                           else _me.get("dano", 0.0))
            # topado como F-DANO se topa: u vive en [0,1] (el techo de volumen
            # del motor, como siempre)
            _u = min(1.0, max(0.0, _dan_r / hp_max))
            _mr = _u
            if _u > CUESTA_UMBRAL_U:          # la cuesta del 43, misma cuenta
                _fr = (_u - CUESTA_UMBRAL_U) / (1.0 - CUESTA_UMBRAL_U)
                _mr = _u + CUESTA_GANANCIA * _fr * _fr
            if _d > _arma.range:
                _mr *= REENCUENTRO_MITAD
            if _mr > _peor_r:
                _peor_r = _mr
                _det_r = {"slot": _sl, "dist": round(_d, 2),
                          "arma": getattr(_arma, "id", None),
                          "dano_recordado": round(_dan_r, 1),
                          "acumulado": round(float(_me.get("acum", 0.0)), 1),
                          "en_alcance": _d <= _arma.range}
        F["F-REENCUENTRO"] = _peor_r
        if _det_r:
            F["_reencuentro"] = _det_r
