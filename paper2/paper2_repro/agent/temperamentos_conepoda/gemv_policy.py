...
# RADIOGRAFÍA DEL CONO (GEMV_CONE_DBG, era del pensamiento enfocado): instrumentación de LECTURA. Reusa el hook
# ctx.autopsy (ya OFF≡bit-exact) para clasificar cada terminal del árbol por dominio, sin tocar la decisión (el
# hook se llama tras computar `d`, su retorno se ignora → conducta idéntica). Muestrea 1 de cada _CONE_EVERY
# ticks. Cuenta por terminal: hacia-el-gain (cono estrecho), cercano (≤_CONE_NEAR, disco del cono abierto), y
# dominio (fuerza mayor F/R/S). Log GEMV_CONE por tick muestreado. OFF (flag apagado) ≡ la vara bit-exact.
_CONE_DBG: bool = os.environ.get("GEMV_CONE_DBG", "0") in ("1", "on", "ON", "true")
_CONE_EVERY: int = int(os.environ.get("GEMV_CONE_EVERY", "10"))
_CONE_NEAR: int = int(os.environ.get("GEMV_CONE_NEAR", "3"))
_cone_acc: dict = {}
# PODA DEL CONO ABIERTO (GEMV_CONE, Pieza 2b): poda el árbol del planner. Conserva ramas hacia el dominante
# (descenso del gfield = dolor SIEMPRE cuando gain[0] es survival), hacia el 2º deseo (gain[1]), o cercanas
# (≤_CONE_NEAR). Poda el resto → menos n_terminal, menos s/step. CAMBIA la conducta (guarda = no-regresión
# de score, no bit-exact ON). OFF (flag apagado) ≡ vara bit-exact (cone_prune=None, el árbol no se toca).
_CONE: bool = os.environ.get("GEMV_CONE", "0") in ("1", "on", "ON", "true")
# Cláusula 'hacia el dominante' — dos modos (barrido del filo, 2026-08-10):
#  OFF (default): descenso-gfield padre→hijo (wall-aware, agresivo, poda ~48%).
#  ON (GEMV_CONE_L1ORIGIN): L1-origen de la RADIOGRAFÍA (`|g0-hijo|<|g0|`, ref. AGENTE; el cono ABIERTO
#  firmado, poda ~30%, fiel al diseño). El near y el dolor-siempre-cerca no cambian.
_CONE_L1O: bool = os.environ.get("GEMV_CONE_L1ORIGIN", "0") in ("1", "on", "ON", "true")
# DRY-RUN del cono-v2 (GEMV_CONE_DRYRUN, PASO CERO frío): instrumentación de LECTURA. Por cada expansión
# del árbol registra en SOMBRA si el predicado v1 (gfield-descent) y el v2 (progreso geodésico a los
# objetivos del cono + gracia de G niveles) podarían la rama, y si la rama cae en una junction GANABLE
# (detour-align relevante). Acumula por profundidad (para calibrar G). Retorno ignorado → conducta idéntica.
# OFF ≡ vara bit-exact. Números: supervivencia de detour-aligns bajo v2 / poda total v2 / G.
_CONE_DRYRUN: bool = os.environ.get("GEMV_CONE_DRYRUN", "0") in ("1", "on", "ON", "true")
_CONE_DRYRUN_EVERY: int = int(os.environ.get("GEMV_CONE_DRYRUN_EVERY", "10"))
_cone_dry_acc: dict = {}
# CONO-V2 ANCHO (GEMV_CONE_V2, paso cero VERDE 2026-08-10): poda por PROGRESO GEODÉSICO hacia CUALQUIER
# junction ganable (campo BFS multi-fuente, wall-aware) + torre (survival) + disco ≤_CONE_NEAR. Conserva
# la conquista de junctions secundarias (el "radar") que el v1 gain[0]-céntrico cegaba. Poda limpia 36.7%,
# recupera ~99.5% de los detour-aligns. OFF ≡ vara bit-exact. El juez real es el banco.
_CONE_V2: bool = os.environ.get("GEMV_CONE_V2", "0") in ("1", "on", "ON", "true")
# PERFIL DE LA FONTANERÍA (GEMV_PROFILE, Fase 0): cProfile de LECTURA sobre choose_action. Vuelca el top de
# funciones por tottime cada _PROFILE_EVERY ticks. Read-only (no cambia la decisión; overhead solo mide).
_PROFILE: bool = os.environ.get("GEMV_PROFILE", "0") in ("1", "on", "ON", "true")
_PROFILE_EVERY: int = int(os.environ.get("GEMV_PROFILE_EVERY", "50"))
if _PROFILE:
    import cProfile as _cProfile
    _cprof = _cProfile.Profile()
# FONTANERÍA ETAPA 1 (GEMV_FAST_PARSE): matar el re-parseo. parse_tokens es función PURA → se memoiza por
# identidad del objeto tokens (guarda `is` contra reuso de id), con LRU acotada, limpiada cada tick. Dentro
# de un nodo, planner+transición+appraisal parsean los MISMOS tokens ~6× → 1×. OFF ≡ bit-exact (sin cache).
# El gate 40/40 caza cualquier mutación oculta del mapa cacheado.
_FAST_PARSE: bool = os.environ.get("GEMV_FAST_PARSE", "0") in ("1", "on", "ON", "true")
if _FAST_PARSE:
    _pt_cache: dict = {}
    _pt_stats: dict = {"hit": 0, "miss": 0}
    _orig_parse_tokens = parse_tokens
    def _fast_parse_tokens(tokens):
        _idk = id(tokens)
        _c = _pt_cache.get(_idk)
        if _c is not None and _c[0] is tokens:
            _pt_stats["hit"] += 1
            return _c[1]
        _pt_stats["miss"] += 1
        _r = _orig_parse_tokens(tokens)
        if len(_pt_cache) > 256:
            _pt_cache.clear()
        _pt_cache[_idk] = (tokens, _r)
        return _r
    import appraisal.appraisal_v3 as _av3_mod, appraisal.appraisal_shelf as _ash_mod
    import appraisal.appraisal_v1 as _av1_mod, appraisal.appraisal_v4 as _av4_mod
    for _pm in (_av3_mod, _ash_mod, _av1_mod, _av4_mod):
        if getattr(_pm, "parse_tokens", None) is not None:
            _pm.parse_tokens = _fast_parse_tokens
    parse_tokens = _fast_parse_tokens        # el nombre del adaptador (usado por ctx.parse_tokens)
...
def _choose_action_impl(
    ...
    """
    if _FAST_PARSE:
        if world_step > 0 and world_step % 30 == 0:
            _h = _pt_stats["hit"]; _m = _pt_stats["miss"]
            _log("GEMV_FASTPARSE step=%d hit=%d miss=%d hitrate=%.1f%%" % (world_step, _h, _m, 100*_h/max(1, _h+_m)))
        _pt_cache.clear()                          # ETAPA 1: cache de parseo por-tick (evita reuso de id entre ticks)
    ...
    if _HORIZON <= 1 and not _PLAN_FORCE:
        ...
        _extract = None
        # RADIOGRAFÍA DEL CONO: clasificador de terminales (solo lee; no cambia `d`) en ticks muestreados.
        _cone_on = False
        if _CONE_DBG and gain is not None and (world_step % _CONE_EVERY == 0):
            _cone_on = True
            _g0c, _g1c = int(gain[0][0]), int(gain[0][1]); _dg0c = abs(_g0c) + abs(_g1c)
            _cone_acc.clear()
            _cone_acc.update(nt=0, toward=0, near=0, tornear=0, domF=0, domR=0, domS=0)
            def _cone_hook(cdr, cdc, d, s, terr, _g0=_g0c, _g1=_g1c, _dg0=_dg0c):
                a = _cone_acc; a["nt"] += 1
                _tw = (abs(_g0 - cdr) + abs(_g1 - cdc)) < _dg0      # reduce la distancia al gain dominante
                _nr = (abs(cdr) + abs(cdc)) <= _CONE_NEAR           # dentro del disco cercano
                if _tw: a["toward"] += 1
                if _nr: a["near"] += 1
                if _tw or _nr: a["tornear"] += 1
                _fF, _fR, _fS = abs(s.pos_F), abs(s.pos_R), abs(s.pos_S)   # dominio = fuerza mayor
                if _fF >= _fR and _fF >= _fS: a["domF"] += 1
                elif _fR >= _fS: a["domR"] += 1
                else: a["domS"] += 1
            ctx.autopsy = _cone_hook
        # PODA DEL CONO ABIERTO (GEMV_CONE): predicado que cierra sobre gain/_gfield. return True = PODAR.
        # Conserva la rama si: (3) cercana al agente (≤_CONE_NEAR), (1) desciende el gfield hacia gain[0]
        # (wall-aware; = ruta de alivio/dolor SIEMPRE cuando gain[0] es survival; respaldo L1 padre→hijo),
        # o (2) acerca al 2º deseo gain[1] (L1 padre→hijo). Poda el resto.
        _cone_prune = None
        if _CONE_V2 and gain is not None:
            # CONO-V2 ANCHO: poda por PROGRESO GEODÉSICO hacia CUALQUIER junction ganable (campo BFS
            # multi-fuente, wall-aware) + torre (survival, _surv_hf desciende) + disco ≤_CONE_NEAR.
            # Conserva la conquista secundaria (el radar). Paso cero VERDE (poda 36.7%, detour-aligns ~99.5%).
            _v2near = _CONE_NEAR
            _own_net_v2 = TAG_NET_COGS if team_side == "cogs" else TAG_NET_CLIPS
            from collections import deque as _dqv2
            _Rv, _Cv = EGOCENTRIC_ROWS, EGOCENTRIC_COLS
            _gfm2 = {}; _qv2 = _dqv2()
            for _lc, _tg2 in tag_map.items():
                if TAG_TYPE_JUNCTION in _tg2 and _own_net_v2 not in _tg2:
                    _jr2 = _lc // _Cv; _jc2 = _lc % _Cv
                    if 0 <= _jr2 < _Rv and 0 <= _jc2 < _Cv and (_jr2 - AGENT_ROW, _jc2 - AGENT_COL) not in _gfm2:
                        _gfm2[(_jr2 - AGENT_ROW, _jc2 - AGENT_COL)] = 0; _qv2.append((_jr2, _jc2, 0))
            while _qv2:
                _rrv, _ccv, _ddv = _qv2.popleft()
                for _drv, _dcv in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                    _nrv, _ncv = _rrv + _drv, _ccv + _dcv
                    if 0 <= _nrv < _Rv and 0 <= _ncv < _Cv:
                        if tag_map.get(_nrv * _Cv + _ncv, ()) and (set(tag_map.get(_nrv * _Cv + _ncv, ())) & _SOLID_TYPES):
                            continue
                        _relv = (_nrv - AGENT_ROW, _ncv - AGENT_COL)
                        if _relv not in _gfm2:
                            _gfm2[_relv] = _ddv + 1; _qv2.append((_nrv, _ncv, _ddv + 1))
            def _cone_prune(nr, nc, pr, pc, _gfm=_gfm2, _shf=_surv_hf, _near=_v2near):
                if abs(nr) + abs(nc) <= _near:
                    return False                                     # disco cercano (dolor local siempre)
                _gc = _gfm.get((nr, nc)); _gp = _gfm.get((pr, pc))
                if _gc is not None and _gp is not None and _gc < _gp:
                    return False                                     # progreso geodésico a ALGUNA junction ganable
                if _shf is not None:                                 # torre: survival — la ruta a casa jamás se poda
                    _sc = _shf.get((nr, nc)); _sp = _shf.get((pr, pc))
                    if _sc is not None and _sp is not None and _sc < _sp:
                        return False
                return True                                          # ni conquista ni casa ni cerca → podar
        elif _CONE and gain is not None:
            # gain = (objetivo (r,c), valor float): no hay 2º deseo-objetivo en machina_1 → el cono es
            # dominante + cercano + dolor-siempre (cláusula 2º-deseo retirada, era código muerto).
            _pg0 = (int(gain[0][0]), int(gain[0][1]))
            _dg0 = abs(_pg0[0]) + abs(_pg0[1])          # dist del AGENTE (origen) a gain[0] (modo L1-origen)
            _pgf = _gfield
            _pnear = _CONE_NEAR
            def _cone_prune(nr, nc, pr, pc, _g0=_pg0, _gf=_pgf, _near=_pnear, _l1o=_CONE_L1O, _dg0=_dg0):
                if abs(nr) + abs(nc) <= _near:
                    return False                                     # (3) disco cercano (dolor local siempre)
                if _l1o:
                    # RADIOGRAFÍA (cono ABIERTO firmado): hacia = hijo más cerca de gain[0] que el AGENTE (origen)
                    return not ((abs(_g0[0] - nr) + abs(_g0[1] - nc)) < _dg0)
                # descenso-gfield padre→hijo (wall-aware; = dolor SIEMPRE cuando gain[0] es survival); respaldo L1 padre→hijo
                if _gf is not None:
                    _gc = _gf.get((nr, nc)); _gp = _gf.get((pr, pc))
                    if _gc is not None and _gp is not None:
                        if _gc < _gp:
                            return False                             # (1) desciende gfield hacia gain[0]
                    elif (abs(_g0[0] - nr) + abs(_g0[1] - nc)) < (abs(_g0[0] - pr) + abs(_g0[1] - pc)):
                        return False                                 # (1) respaldo L1 fuera del gfield
                elif (abs(_g0[0] - nr) + abs(_g0[1] - nc)) < (abs(_g0[0] - pr) + abs(_g0[1] - pc)):
                    return False                                     # (1) sin gfield: L1 hacia gain[0]
                return True                                          # fuera del cono → podar
        # DRY-RUN v2 (GEMV_CONE_DRYRUN): callback de LECTURA — registra v1 vs v2 por expansión, por depth.
        _cone_dryrun = None; _dry_on = False
        if _CONE_DRYRUN and gain is not None and (world_step % _CONE_DRYRUN_EVERY == 0):
            _dry_on = True
            _cone_dry_acc.clear()
            _cone_dry_acc.update(n={}, v1p={}, v2p2={}, v2p3={}, jn=0, j_v1p=0, j_v1p_v2keep=0, j_v1pdepth={},
                                 widep={}, j_v1p_toG0=0, j_v1p_gfnone=0, geowidep={}, v1p_geokeep=0)
            _dg0d = (int(gain[0][0]), int(gain[0][1])); _dgf = _gfield; _dnear = _CONE_NEAR
            _own_net_d = TAG_NET_COGS if team_side == "cogs" else TAG_NET_CLIPS
            # posiciones root-relativas de las junctions GANABLES (para "se dirige a una junction")
            _jpos = []
            for _loc, _tg in tag_map.items():
                if TAG_TYPE_JUNCTION in _tg and _own_net_d not in _tg:
                    _jpos.append((_loc // EGOCENTRIC_COLS - AGENT_ROW, _loc % EGOCENTRIC_COLS - AGENT_COL))
            # CAMPO GEODÉSICO MULTI-FUENTE: BFS wall-aware desde TODAS las junctions ganables → dist geodésica
            # a la MÁS CERCANA. Es el predicado del cono ANCHO limpio (no-L1). ~208 celdas: barato.
            from collections import deque as _dq2
            _R2, _C2 = EGOCENTRIC_ROWS, EGOCENTRIC_COLS
            _gfmulti = {}; _q2 = _dq2()
            for (_jr, _jc) in _jpos:
                _er0 = AGENT_ROW + _jr; _ec0 = AGENT_COL + _jc
                if 0 <= _er0 < _R2 and 0 <= _ec0 < _C2 and (_jr, _jc) not in _gfmulti:
                    _gfmulti[(_jr, _jc)] = 0; _q2.append((_er0, _ec0, 0))
            while _q2:
                _rr, _cc, _dd = _q2.popleft()
                for _dr2, _dc2 in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                    _nr2, _nc2 = _rr + _dr2, _cc + _dc2
                    if 0 <= _nr2 < _R2 and 0 <= _nc2 < _C2:
                        if tag_map.get(_nr2 * _C2 + _nc2, ()) and (set(tag_map.get(_nr2 * _C2 + _nc2, ())) & _SOLID_TYPES):
                            continue
                        _rel2 = (_nr2 - AGENT_ROW, _nc2 - AGENT_COL)
                        if _rel2 not in _gfmulti:
                            _gfmulti[_rel2] = _dd + 1; _q2.append((_nr2, _nc2, _dd + 1))
            def _cone_dryrun(nr, nc, pr, pc, depth, _g0=_dg0d, _gf=_dgf, _near=_dnear, _jp=_jpos,
                             _gfm=_gfmulti, _a=_cone_dry_acc):
                # v1 real: gfield-descent estricto (respaldo L1 fuera de campo)
                if abs(nr) + abs(nc) <= _near:
                    v1p = False
                elif _gf is not None and _gf.get((nr, nc)) is not None and _gf.get((pr, pc)) is not None:
                    v1p = not (_gf.get((nr, nc)) < _gf.get((pr, pc)))
                else:
                    v1p = not ((abs(_g0[0] - nr) + abs(_g0[1] - nc)) < (abs(_g0[0] - pr) + abs(_g0[1] - pc)))
                # v2 con gracia G: keep si depth<=G, near, gfield NO-sube (progreso/meseta), o L1 no-sube
                def _v2p(G):
                    if depth <= G:
                        return False
                    if abs(nr) + abs(nc) <= _near:
                        return False
                    if _gf is not None and _gf.get((nr, nc)) is not None and _gf.get((pr, pc)) is not None:
                        if _gf.get((nr, nc)) <= _gf.get((pr, pc)):
                            return False
                    if (abs(_g0[0] - nr) + abs(_g0[1] - nc)) <= (abs(_g0[0] - pr) + abs(_g0[1] - pc)):
                        return False
                    return True
                v2p2 = _v2p(2); v2p3 = _v2p(3)
                _a['n'][depth] = _a['n'].get(depth, 0) + 1
                if v1p: _a['v1p'][depth] = _a['v1p'].get(depth, 0) + 1
                if v2p2: _a['v2p2'][depth] = _a['v2p2'].get(depth, 0) + 1
                if v2p3: _a['v2p3'][depth] = _a['v2p3'].get(depth, 0) + 1
                # "se dirige a una junction ganable": el hijo acerca (L1) a ALGUNA junction respecto al padre
                _head = False
                if _jp:
                    _dc = min(abs(j[0] - nr) + abs(j[1] - nc) for j in _jp)
                    _dp = min(abs(j[0] - pr) + abs(j[1] - pc) for j in _jp)
                    _head = _dc < _dp
                # CONO ANCHO L1 (A, cota superior): poda si NO cerca y NO se dirige (L1) a ninguna junction
                if not ((abs(nr) + abs(nc) <= _near) or _head):
                    _a['widep'][depth] = _a['widep'].get(depth, 0) + 1
                # CONO ANCHO GEODÉSICO (limpio, no-L1): keep si cerca o gfmulti DESCIENDE (progreso wall-aware
                # hacia la junction ganable más cercana). Es el predicado DEFINITIVO del v2-ancho.
                _gcm = _gfm.get((nr, nc)); _gpm = _gfm.get((pr, pc))
                _geokeep = (abs(nr) + abs(nc) <= _near) or (_gcm is not None and _gpm is not None and _gcm < _gpm)
                if not _geokeep:
                    _a['geowidep'][depth] = _a['geowidep'].get(depth, 0) + 1
                if v1p and _geokeep:                                    # NO-CIRCULAR: v1 la mata, el geodésico la salva
                    _a['v1p_geokeep'] += 1
                if _head:                                                   # rama que progresa hacia una junction
                    _a['jn'] += 1
                    if v1p:                                                 # ...y v1 la mata (candidata a detour-align)
                        _a['j_v1p'] += 1
                        _a['j_v1pdepth'][depth] = _a['j_v1pdepth'].get(depth, 0) + 1
                        if not v2p2: _a['j_v1p_v2keep'] += 1               # v2(spec) la salva
                        # A/B: ¿va a gain[0] (rodeo real, B)? ¿gfield None en el hijo (fuera de ventana, culpable 3)?
                        if (abs(_g0[0] - nr) + abs(_g0[1] - nc)) < (abs(_g0[0] - pr) + abs(_g0[1] - pc)):
                            _a['j_v1p_toG0'] += 1
                        if _gf is None or _gf.get((nr, nc)) is None:
                            _a['j_v1p_gfnone'] += 1
        if _TIEBREAK:
            _pt = plan_terminal(
                ...
                gain_field=_gfield, commit_k=_ck, gain_solids=_gsolids, cone_prune=_cone_prune, cone_dryrun=_cone_dryrun,
            )
        ...
        if _cone_on:                                          # RADIOGRAFÍA: volcar el mapa del tick y limpiar el hook
            _log("GEMV_CONE " + json.dumps({"step": world_step, "grel": [_g0c, _g1c], **_cone_acc}))
            ctx.autopsy = None
        if _dry_on:                                           # DRY-RUN v2: volcar los acumuladores del tick
            _a = _cone_dry_acc
            _log("GEMV_CONE_DRYRUN " + json.dumps({"step": world_step,
                 "n": _a["n"], "v1p": _a["v1p"], "v2p2": _a["v2p2"], "v2p3": _a["v2p3"],
                 "jn": _a["jn"], "j_v1p": _a["j_v1p"], "j_v1p_v2keep": _a["j_v1p_v2keep"],
                 "j_v1pdepth": _a["j_v1pdepth"], "widep": _a["widep"],
                 "j_v1p_toG0": _a["j_v1p_toG0"], "j_v1p_gfnone": _a["j_v1p_gfnone"],
                 "geowidep": _a["geowidep"], "v1p_geokeep": _a["v1p_geokeep"]}))
...
def choose_action(*a, **k):
    """Envoltorio de perfil (GEMV_PROFILE). OFF → llamada directa al impl (cero overhead, bit-exact)."""
    if not _PROFILE:
        return _choose_action_impl(*a, **k)
    _cprof.enable()
    try:
        return _choose_action_impl(*a, **k)
    finally:
        _cprof.disable()
        _ws = k.get("world_step", 0)
        if _ws > 0 and _ws % _PROFILE_EVERY == 0:
            import pstats as _pstats, io as _io
            _sio = _io.StringIO()
            _pstats.Stats(_cprof, stream=_sio).sort_stats("tottime").print_stats(18)
            _log("GEMV_PROFILE step=%d\n%s" % (_ws, _sio.getvalue()))
