...
import os as _os
# FONTANERÍA ETAPA 2 (GEMV_ARRAY): scalar_map como ARRAY plano indexado por loc·NFID+fid (el duelo dio
# +25% sobre dict.get: el builtin construía la tupla por consulta). El plumbing (parse/get/iter) es TOCABLE;
# el CÁLCULO intocable. OFF ≡ dict de siempre. Gate 40/40 = notario. NFID/NLOC dimensionan el grid.
_ARRAY_STATE: bool = _os.environ.get("GEMV_ARRAY", "0") in ("1", "on", "ON", "true")
_NLOC = 256
_NFID = 74

class GridState:
    """scalar_map respaldado por array plano. `arr[loc*NFID+fid]`. `order`/`present` para iteración y .get
    con default ≠ 0 (los ~5 sitios directos del adaptador). Duck-typea el dict donde haga falta."""
    __slots__ = ("arr", "order", "present")
    def __init__(self, arr, order, present):
        self.arr = arr; self.order = order; self.present = present
    def get(self, key, default=0):
        return self.arr[key[0] * _NFID + key[1]] if key in self.present else default
    ...
    def items(self):
        a = self.arr
        for (loc, fid) in self.order:
            yield (loc, fid), a[loc * _NFID + fid]
    def keys(self):
        return iter(self.order)
    def __len__(self):
...
def parse_tokens(
    ...
    if _ARRAY_STATE:                                       # ETAPA 2: scalar_map como GridState (array plano)
        arr = [0] * (_NLOC * _NFID); order = []; present = set()
        for loc, fid, val in tokens:
            ...
            if fid == FID_TAG:
                ...
                arr[loc * _NFID + fid] = val
                k = (loc, fid)
                if k not in present:
                    present.add(k); order.append(k)
        return GridState(arr, order, present), tag_map
...
def get_scalar(
    ...
    if _ARRAY_STATE:                                       # ETAPA 2: array-inline
        idx = loc * _NFID + fid
        if default == 0:
            return scalar_map.arr[idx]
        return scalar_map.arr[idx] if (loc, fid) in scalar_map.present else default
...
def get_multipart(
    ...
    if _ARRAY_STATE:                                       # ETAPA 2: array-inline (sin construir tupla)
        a = scalar_map.arr; base = loc * _NFID + base_fid
        if n_parts == 2:
            return a[base] + 256 * a[base + 1]
        value = 0
        for i in range(n_parts):
            value += a[base + i] * (256 ** i)
        return value
    value = 0
    for i in range(n_parts):
