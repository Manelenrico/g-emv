...
def _get_multipart(tokens, loc, fid_p0):
    p0 = p1 = 0
    for t in tokens:
...
def make_heart_bump(tokens, ctx: TransCtx):
    ...
    if not team_fids:
        return None
    hearts = _get_multipart(tokens, ctx.agent_loc, ctx.fid_inv_heart)
    if ctx.heart_cap is not None and hearts >= ctx.heart_cap:
        return None
    pantry_ok = all(_get_multipart(tokens, ctx.team_loc, tf) >= ctx.heart_recipe for tf in team_fids)
...
def equip_aligner_bump(tokens, ctx: TransCtx):
    ...
    if not ctx.model_marcador or ctx.fid_inv_aligner is None:
        return None
    if _get_multipart(tokens, ctx.agent_loc, ctx.fid_inv_aligner) > 0:
    ...
    if getattr(ctx, "equip_gated", False):
        ...
        for tf, c in ctx.aligner_cost.items():
            if _get_multipart(tokens, ctx.team_loc, tf) < c:
                return None
    out = tokens
...
def align_junction_bump(tokens, dest_loc, ctx: TransCtx):
    ...
    if _get_multipart(tokens, ctx.agent_loc, ctx.fid_inv_aligner or -1) < 1:
        return None                                   # sin aligner
    if _get_multipart(tokens, ctx.agent_loc, ctx.fid_inv_heart or -1) < 1:
    ...
    hearts = _get_multipart(out, ctx.agent_loc, ctx.fid_inv_heart)
...
def station_bump_effect(tokens, action, ctx: TransCtx):
    ...
    if ctx.model_marcador and ctx.tag_type_junction in tags:
        aligned = align_junction_bump(tokens, dloc, ctx)
    ...
    if ctx.model_marcador and ctx.tag_type_aligner_station is not None and ctx.tag_type_aligner_station in tags:
        equipped = equip_aligner_bump(tokens, ctx)
    ...
    if (ctx.model_deposit and ctx.tag_type_hub in tags and ctx.own_team_tag in tags):
        ...
        made = make_heart_bump(tokens, ctx)
        if made is not None:
            return made, "make"
